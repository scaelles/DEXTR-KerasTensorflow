...
from networks.classifiers import build_pyramid_pooling_module, build_atrous_pooling_module
...
def basic_block(input_tensor, kernel_size, filters, stage, block, dilation=1, stride=(1, 1), downsample=False):
    """The basic block for small resnets
        ...
        # Arguments
            ...
            dilation: dilation of the convolutions
            dow
        # Returns
    ...
    x = Conv2D(filters2, kernel_size, name=conv_name_base + '2a', use_bias=False,
               strides=stride, padding='valid')(x)
    x = BN(axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)

    x = Conv2D(filters2, kernel_size, padding='same', name=conv_name_base + '2b', use_bias=False,
               dilation_rate=dilation)(x)
    x = BN(axis=bn_axis, name=bn_name_base + '2b')(x)

    if downsample:
        shortcut = Conv2D(filters2, (1, 1), strides=stride, name=conv_name_base + '1', use_bias=False)(input_tensor)
...
def ResNet18(input_tensor=None):
    ...
    x = Conv2D(64, (7, 7), strides=(2, 2), name='conv1', use_bias=False, padding='valid')(x)
    ...
    x = basic_block(x, 3, [128, 128], stage=3, block='a', stride=2, downsample=True)
    x = basic_block(x, 3, [128, 128], stage=3, block='b')

    x = basic_block(x, 3, [256, 256], stage=4, block='a', dilation=2, downsample=True)
    x = basic_block(x, 3, [256, 256], stage=4, block='b', dilation=2)

    x = basic_block(x, 3, [512, 512], stage=5, block='a', dilation=4, downsample=True)
    x = basic_block(x, 3, [512, 512], stage=5, block='b', dilation=4)
...
def build_network(nb_classes, input_shape, resnet_layers=101, classifier='psp', sigmoid=False, output_size=None,
    ...
    elif classifier == 'atrous':
        print("Building network based on ResNet %i and Atrous module expecting inputs of shape %s predicting %i classes" % (
            resnet_layers, input_shape, nb_classes))
        x = build_atrous_pooling_module(res, nb_classes, sigmoid=sigmoid, output_size=output_size)
    else:
